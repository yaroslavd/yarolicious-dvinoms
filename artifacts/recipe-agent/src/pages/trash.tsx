...
import { Loader2, Trash2, RotateCcw, BookOpen, Salad, GitBranch } from "lucide-react";
import type { TrashedRecipe, TrashedProfile, TrashedVersion } from "@workspace/api-client-react";
  ...
  async function handleRestore(target: ConfirmTarget) {
    try {
      if (target.type === "recipe") {
        ...
        await restoreVersion.mutateAsync({ id: target.item.recipeId, versionId: target.item.id });
  ...
  async function handleHardDelete() {
    ...
    try {
      if (confirmTarget.type === "recipe") {
        ...
        toast({ title: `"${(confirmTarget.item as TrashedRecipe).name}" permanently deleted` });
      } else if (confirmTarget.type === "profile") {
        await hardDeleteProfile.mutateAsync({ id: confirmTarget.item.id });
        toast({ title: `"${(confirmTarget.item as TrashedProfile).name}" permanently deleted` });
      } else {
        await hardDeleteVersion.mutateAsync({ id: confirmTarget.item.id });
        toast({ title: `Version "${(confirmTarget.item as TrashedVersion).label}" permanently deleted` });
      }
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      ...
      {isEmpty && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-muted-foreground">Nothing here yet. Deleted items will appear here.</p>
        </div>
      )}
          ...
          <div className="space-y-2">
            {trash.recipes.map((recipe) => (
              <TrashItem
                ...
                onRestore={() => handleRestore({ type: "recipe", item: recipe })}
                onDeleteForever={() => setConfirmTarget({ type: "recipe", item: recipe })}
          ...
          <div className="space-y-2">
            {trash.profiles.map((profile) => (
              <TrashItem
                ...
                onRestore={() => handleRestore({ type: "profile", item: profile })}
                onDeleteForever={() => setConfirmTarget({ type: "profile", item: profile })}
                disabled={isAnyPending}
              />
            ))}
          </div>
          ...
          <div className="space-y-2">
            {trash.versions.map((version) => (
              <TrashItem
                ...
                onRestore={() => handleRestore({ type: "version", item: version })}
                onDeleteForever={() => setConfirmTarget({ type: "version", item: version })}
                disabled={isAnyPending}
              />
            ))}
          </div>
      ...
      <AlertDialog open={!!confirmTarget} onOpenChange={(open) => { if (!open) setConfirmTarget(null); }}>
        <AlertDialogContent>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold">
                "{confirmTarget?.type === "recipe"
                  ...
                  : (confirmTarget?.item as TrashedVersion | undefined)?.label}"
              </span>{" "}
          ...
          <AlertDialogFooter>
            ...
            >
              {hardDeleteRecipe.isPending || hardDeleteProfile.isPending || hardDeleteVersion.isPending ? (
...
function TrashItem({ title, subtitle, deletedAt, onRestore, onDeleteForever, disabled }: TrashItemProps) {
  return (
    <div className="flex items-center gap-4 p-4 rounded-xl border border-border/60 bg-card hover:bg-card/80 transition-colors">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground/70 mt-0.5">Deleted {timeAgo(deletedAt)}</p>
      </div>
