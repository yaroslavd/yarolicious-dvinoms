...
function FixComplianceDialog({
  recipeId,
  scores,
  profiles,
  onClose,
  onVersionSaved,
}: FixComplianceDialogProps) {
  const { toast } = useToast();
  const nonCompliantProfileIds = scores
    .filter((s) => s.score < 80)
    .map((s) => s.profileId);
  const [selectedProfileIds, setSelectedProfileIds] = useState<number[]>(
    nonCompliantProfileIds,
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const message: string =
        (err?.data as any)?.error ?? err.message ?? "Could not save version";
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {score && (
                        <span
                          className={`text-xs font-bold ${getScoreColor(score.score).text}`}
                        >
                          {score.score}%
                        </span>
                      )}
                    ...
                    {isPerfect && (
                      <p className="text-xs text-emerald-700 dark:text-emerald-400 font-medium">
                        This recipe already fully meets this profile — no
                        changes needed.
                      </p>
                    )}
                    {isZero && (
                      <p className="text-xs text-red-600 dark:text-red-400 font-medium">
                        This recipe doesn't meet this profile at all.
            ...
            <div className="space-y-2">
              ...
              {projectedScores.map((ps) => (
                ...
                >
                  ...
                  <span
                    className={`font-bold ${getScoreColor(ps.scoreBefore).text}`}
                  >
                    {ps.scoreBefore}%
                  </span>
                  <ArrowRight className="w-4 h-4 text-muted-foreground" />
                  <span
                    className={`font-bold ${getScoreColor(ps.scoreAfter).text}`}
                  >
                    {ps.scoreAfter}%
                  </span>
                </div>
          ...
          <div className="space-y-2">
            ...
            {suggestions.length === 0 ? (
              ...
              suggestions.map((s, i) => (
                <div
                  key={i}
                  className="p-3 rounded-xl border border-border/60 bg-muted/30 space-y-1"
                >
      ...
      <div className="grid gap-3">
        {scores.map((score) => {
          ...
          return (
            <div
              key={score.profileId}
              className="p-4 rounded-xl border border-border/60 bg-background/60"
            >
...
export default function RecipeDetail() {
  ...
  const handleVersionSaved = useCallback(() => {
    setTimeout(() => {
      versionSwitcherRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }, 400);
  ...
  const { data: baseComplianceScores } = useRecipeComplianceScoresForVersion(
    id,
    null,
  );
  const {
  ...
  } = useRecipeComplianceScoresForVersion(id, selectedVersionId);
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="rounded-lg text-destructive hover:bg-destructive hover:text-destructive-foreground border-destructive/20"
              >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  The recipe will be moved to Trash. You can restore it from
                  there at any time, or permanently delete it.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <div className="p-6 md:p-10 -mt-10 relative z-10">
              ...
              {recipe.categories && (
                <div className="flex flex-wrap gap-2">
                  {recipe.categories.split(",").map((cat, i) => (
                    <span
                      key={i}
                      className="px-3 py-1 bg-accent text-accent-foreground text-xs font-semibold rounded-full uppercase tracking-wider"
                    >
                      {cat.trim()}
                    </span>
                  ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-6 border-y border-border/50 mb-10">
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" /> Prep
              </span>
            ...
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" /> Cook
              </span>
              <p className="font-medium text-foreground">
                {recipe.cookTime || "—"}
              </p>
            ...
            <div className="space-y-1">
              <span className="text-xs text-primary uppercase tracking-wider font-bold flex items-center gap-1.5">
            ...
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold flex items-center gap-1.5">
                <Users className="w-3.5 h-3.5" /> Yield
              </span>
              <p className="font-medium text-foreground">
                {recipe.servings || "—"}
              </p>
              ...
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  <span className="font-semibold">
                    "{versionToDelete?.label}"
                  </span>{" "}
                  version will be moved to Trash. You can restore it from the
                  Trash page at any time.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-1">
              ...
              <ul className="space-y-3 font-sans">
                {displayIngredients
                  ...
                  .map((ing, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-foreground pb-3 border-b border-border/40 last:border-0"
                    >
              ...
              {recipe.originType === "ai_generated" &&
                recipe.generationPrompt && (
                  <div className="mt-10 p-4 bg-muted/50 rounded-xl">
                    <p className="text-sm font-semibold text-foreground mb-2">
                      Origin
                    </p>
